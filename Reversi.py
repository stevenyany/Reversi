...
class ReversiSquare(Canvas):
    ...
    def set_highlight(self,flag):
        '''ReversiSquare.set_highlight(flag)
        sets highlight color, red if flag is True, background white if flag is False'''
        if flag:
    ...
    def unhighlight(self):
        '''ReversiSquare.unhighlight()
        changes highlight color to white'''
        self['highlightbackground'] = 'white'
...
class ReversiGame(Frame):
    ...
    def update_display(self):
        ...
        # update squares
        legal_moves = self.board.get_legal_moves()  # highlight these squares
        for row in range(8):
            for column in range(8):
                rc = (row,column)
                ...
                # set the square highlight depending whether its a legal move
                if (self.board.get_player() != self.computerPlayer and self.computerPlayer is not None) \
                    or (self.board.get_player() == self.computerPlayer and self.computerPlayer is None) \
                        or (self.board.get_player() != self.computerPlayer and self.computerPlayer is None):
                    self.squares[rc].set_highlight(rc in legal_moves)
                else:
                    self.squares[rc].unhighlight()
